...
@category_bp.route('/sports', methods=['POST'])
def create_sports_category():
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'message': 'Sports category created successfully', 'id': new_category.id}), 201
    except Exception as e:
        db.session.rollback()
        return jsonify({'message': 'Failed to create sports category', 'error': str(e)}), 500
